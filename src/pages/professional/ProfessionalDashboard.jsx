...
export default function ProfessionalDashboard() {
  ...
  const getSkillLevel = () => {
    if (correctSubmissions >= 50) return {
      level: 'Expert',
      color: 'text-primary'
    };
    if (correctSubmissions >= 20) return {
      level: 'Advanced',
      color: 'text-easy'
    };
    if (correctSubmissions >= 5) return {
      level: 'Intermediate',
      color: 'text-warning'
    };
    return {
  ...
  return <div className="page-container">
      ...
      <div>
        ...
        {challenges.length > 0 ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {challenges.map(assessment => <AssessmentCard key={assessment.id} assessment={assessment} onClick={() => navigate(`/professional/challenges/${assessment.id}`)} />)}
