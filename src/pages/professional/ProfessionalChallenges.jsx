...
export default function ProfessionalChallenges() {
  ...
  return <div className="page-container">
      ...
      {filteredAssessments.length > 0 ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredAssessments.map(assessment => <AssessmentCard key={assessment.id} assessment={assessment} onClick={() => navigate(`/professional/challenges/${assessment.id}`)} />)}
        </div> : <div className="card-elevated text-center py-12"><Flame className="w-12 h-12 text-muted-foreground mx-auto mb-4" /><p className="text-muted-foreground">No challenges match your filters</p></div>}
    </div>;
}