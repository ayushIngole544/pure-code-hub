...
export default function ProfessionalPractice() {
  ...
  return <div className="page-container">
      ...
      {practiceAssessments.length > 0 ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {practiceAssessments.map(assessment => <AssessmentCard key={assessment.id} assessment={assessment} onClick={() => navigate(`/professional/challenges/${assessment.id}`)} />)}
        </div> : <div className="card-elevated text-center py-12"><BookOpen className="w-12 h-12 text-muted-foreground mx-auto mb-4" /><p className="text-muted-foreground">No practice problems available</p></div>}
    </div>;
}