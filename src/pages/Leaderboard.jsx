...
export default function Leaderboard() {
  ...
  useEffect(() => {
    async function fetchLeaderboard() {
      ...
      const {
        data: submissions
      } = await supabase.from('submissions').select('student_id, is_correct');
      if (!submissions) {
        setLoading(false);
        return;
      }
      ...
      submissions.forEach(s => {
        if (!statsMap[s.student_id]) statsMap[s.student_id] = {
          total: 0,
          correct: 0
        };
  ...
  const getRankIcon = index => {
    ...
    if (index === 2) return <Award className="w-6 h-6 text-warning" style={{
      opacity: 0.7
    }} />;
  ...
  return <>
      ...
      <div className="page-container">
        ...
        {loading ? <div className="flex justify-center py-20">
            ...
            {/* Top 3 cards */}
            {entries.length >= 3 && <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                {entries.slice(0, 3).map((entry, i) => <div key={entry.user_id} className={`card-elevated text-center ${i === 0 ? 'border-warning/50 bg-warning-light/30' : ''}`}>
            ...
            <div className="card-elevated overflow-hidden p-0">
              <table className="w-full">
                ...
                <tbody className="divide-y divide-border">
                  {entries.map((entry, i) => <tr key={entry.user_id} className={`hover:bg-secondary/30 transition-colors ${i < 3 ? 'bg-warning-light/10' : ''}`}>
