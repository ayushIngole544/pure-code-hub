  ...
  const generateWithAI = async () => {
    ...
    try {
      ...
      const generatedQuestion = {
        ...
        description: aiDifficulty === 'easy' ? 'Given an array of integers, return the sum of all elements.' : aiDifficulty === 'medium' ? 'Given an array of integers and a target, return indices of two numbers that add up to the target.' : 'Given a collection of intervals, merge all overlapping intervals.',
        starter_code: getStarterCode(aiLanguage),
        language: aiLanguage,
        test_cases: [{
          input: aiDifficulty === 'easy' ? '[1, 2, 3]' : '[2, 7, 11, 15], 9',
          expectedOutput: aiDifficulty === 'easy' ? '6' : '[0, 1]'
        }, {
          input: aiDifficulty === 'easy' ? '[10, 20, 30]' : '[3, 2, 4], 6',
          expectedOutput: aiDifficulty === 'easy' ? '60' : '[1, 2]'
        }]
      };
          ...
          <div className="card-elevated">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="md:col-span-2">
                ...
                <input type="text" value={title} onChange={e => setTitle(e.target.value)} className="input-field" placeholder="e.g., JavaScript Fundamentals" required />
              </div>

              <div className="md:col-span-2">
                <label className="label-text">Description</label>
                <textarea value={description} onChange={e => setDescription(e.target.value)} className="input-field min-h-[100px] resize-none" placeholder="Describe what this assessment covers..." required />
              </div>
              ...
              <div>
                ...
                <input type="number" value={timeLimit || ''} onChange={e => setTimeLimit(e.target.value ? parseInt(e.target.value) : undefined)} className="input-field" placeholder="No limit" min={5} max={180} />
              </div>

              <div className="flex items-center gap-3 pt-6">
                <input type="checkbox" id="isPublished" checked={isPublished} onChange={e => setIsPublished(e.target.checked)} className="w-4 h-4 rounded border-border text-primary focus:ring-primary" />
          ...
          <div className="card-elevated">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex gap-2">
                <button type="button" onClick={() => setShowAIPanel(!showAIPanel)} className="flex items-center gap-2 btn-secondary">
                  <Sparkles className="w-4 h-4" />
                  AI Generate
                </button>
                <button type="button" onClick={addQuestion} className="flex items-center gap-2 btn-primary">
            ...
            {showAIPanel && <div className="bg-secondary/50 rounded-lg p-4 mb-6 border border-border">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                  <div>
                    <label className="label-text">Difficulty</label>
                    <select value={aiDifficulty} onChange={e => setAiDifficulty(e.target.value)} className="select-field">
                      <option value="easy">Easy</option>
                      <option value="medium">Medium</option>
                      <option value="hard">Hard</option>
                    </select>
            ...
            {questions.length === 0 ? <div className="text-center py-8 text-muted-foreground">
                <p>No questions added yet. Use the buttons above to add questions.</p>
              </div> : <div className="space-y-4">
                {questions.map((question, index) => <div key={index} className="border border-border rounded-lg p-4">
                    ...
                    <div className="grid gap-4">
                      <div>
                        ...
                        <input type="text" value={question.title} onChange={e => updateQuestion(index, {
                    title: e.target.value
                  })} className="input-field" placeholder="e.g., Reverse a String" required />
                      </div>
                      <div>
                        <label className="label-text">Description</label>
                        <textarea value={question.description} onChange={e => updateQuestion(index, {
                    description: e.target.value
                  })} className="input-field min-h-[80px] resize-none" placeholder="Describe the problem..." required />
                      </div>
                      <div>
                        <label className="label-text">Starter Code</label>
                        <textarea value={question.starter_code} onChange={e => updateQuestion(index, {
