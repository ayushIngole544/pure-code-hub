...
export default function TeacherStudents() {
  ...
  useEffect(() => {
    ...
    supabase.from('profiles').select('user_id, name').in('user_id', studentIds).then(({
      data
    }) => {
      if (data) {
        const map = {};
  ...
  const studentStats = studentSubmissions.reduce((acc, sub) => {
    ...
    if (sub.is_correct) acc[sub.student_id].correct++;else acc[sub.student_id].wrong++;
  ...
  return <div className="page-container">
      ...
      {students.length > 0 ? <div className="card-elevated overflow-hidden p-0">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-border">
              {students.map(student => <tr key={student.id} className="hover:bg-secondary/30 transition-colors">
