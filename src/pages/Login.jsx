        ...
        <div className="w-full max-w-md">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-muted-foreground mt-2">
              {isSignup ? 'Start your coding journey today' : 'Sign in to continue to CodeHub'}
            </p>
          </div>

          {error && <div className="mb-6 p-4 bg-error-light rounded-lg flex items-center gap-3 text-error">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            {isSignup && <div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  {['student', 'teacher', 'professional'].map(r => <button key={r} type="button" onClick={() => setRole(r)} className={`py-3 px-4 rounded-lg border-2 text-sm font-medium capitalize transition-all ${role === r ? 'border-primary bg-primary/5 text-primary' : 'border-border text-muted-foreground hover:border-primary/50'}`}>
                      {r}
                    </button>)}
                </div>
              </div>}
