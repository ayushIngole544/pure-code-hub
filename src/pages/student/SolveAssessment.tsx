...
import { ArrowLeft, Clock, CheckCircle, XCircle, ChevronLeft, ChevronRight } from 'lucide-react';

export default function SolveAssessment() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const { user } = useAuth();
  const { assessments, addSubmission } = useData();
    ...
    return (
      <div className="page-container">
        <div className="card-elevated text-center py-12">
          ...
          <button onClick={() => navigate('/student/assessments')} className="btn-primary mt-4">
            Back to Assessments
          </button>
        </div>
      </div>
  ...
  const handleSubmit = (code: string) => {
    ...
    // Simulated evaluation - randomly determine if correct (70% success rate for demo)
        ...
        <div className="max-w-2xl mx-auto">
          <div className="card-elevated text-center">
            <div className={`w-20 h-20 mx-auto mb-6 rounded-full flex items-center justify-center ${
              correctCount / totalAnswered >= 0.7 ? 'bg-success-light' : 'bg-warning-light'
            }`}>
              {correctCount / totalAnswered >= 0.7 ? (
                <CheckCircle className="w-10 h-10 text-easy" />
              ) : (
            ...
            <div className="grid grid-cols-3 gap-4 mb-8 p-6 bg-secondary/50 rounded-lg">
              <div>
                <p className="text-3xl font-bold text-easy">{correctCount}</p>
                <p className="text-sm text-muted-foreground">Correct</p>
              </div>
              <div>
                <p className="text-3xl font-bold text-error">{totalAnswered - correctCount}</p>
                <p className="text-sm text-muted-foreground">Incorrect</p>
              </div>
              <div>
                <p className="text-3xl font-bold text-foreground">
                  {((correctCount / totalAnswered) * 100).toFixed(0)}%
                </p>
          ...
          <div className="flex items-center gap-4">
            {timeRemaining !== null && (
              <div className={`flex items-center gap-2 px-3 py-1.5 rounded-lg ${
                timeRemaining < 300 ? 'bg-error-light text-error' : 'bg-secondary text-foreground'
              }`}>
                ...
                <div
                  ...
                  className={`w-8 h-8 rounded-lg flex items-center justify-center text-sm font-medium ${
                    i === currentQuestionIndex
                      ...
                      : results[assessment.questions[i].id] !== undefined
                      ? results[assessment.questions[i].id]
                        ? 'bg-success-light text-easy'
