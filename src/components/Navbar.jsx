      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          {isAuthenticated && <div className="hidden md:flex items-center gap-1">
              {getNavLinks().map(link => <Link key={link.path} to={link.path} className={`nav-link ${isActive(link.path) ? 'nav-link-active' : ''}`}>
                  {link.label}
                </Link>)}
            </div>}
        ...
        {/* Mobile navigation */}
        {isAuthenticated && <div className="md:hidden flex overflow-x-auto pb-2 -mx-4 px-4 gap-1">
            {getNavLinks().map(link => <Link key={link.path} to={link.path} className={`nav-link whitespace-nowrap text-sm ${isActive(link.path) ? 'nav-link-active' : ''}`}>
                {link.label}
              </Link>)}
          </div>}
      </div>
